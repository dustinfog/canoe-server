...
import com.googlecode.canoe.fcgi.message.FCGIBeginRequest;
import com.googlecode.canoe.fcgi.message.FCGIEndRequest;
import com.googlecode.canoe.fcgi.message.FCGIHeader;
import com.googlecode.canoe.fcgi.message.FCGIParams;
import com.googlecode.canoe.fcgi.message.FCGIStdin;
import com.googlecode.canoe.fcgi.util.IoUtils;

/**
 *
 * @author panzd
 */
public class FCGIConnection implements GatheringByteChannel,
		ScatteringByteChannel, ByteChannel {

	private final static ConcurrentLinkedQueue<FCGIConnection> connPool = new ConcurrentLinkedQueue<FCGIConnection>();

	public static FCGIConnection open() {
		FCGIConnection conn = connPool.poll();
		if (conn == null)
			conn = new FCGIConnection();

		return conn;
	}

	private static final String DEFAULT_CONTENT_TYPE = "application/x-www-form-urlencoded";
	private static final String PARAM_SCRIPT_FILENAME = "SCRIPT_FILENAME";
	private static final String PARAM_QUERY_STRING = "QUERY_STRING";
	private static final String PARAM_CONTENT_TYPE = "CONTENT_TYPE";
	private static final String PARAM_CONTENT_LENGTH = "CONTENT_LENGTH";
	private static final String PARAM_REQUEST_METHOD = "REQUEST_METHOD";
	...
	public void connect(SocketAddress fcgiRemote) throws IOException {
		if(isOpen())
			throw new RuntimeException("this conn has not closed yet ");

		socketChannel = SocketChannel.open();
		socketChannel.connect(fcgiRemote);
	}

	public void beginRequest(String scriptFilename, boolean keepAlive)
			throws IOException {
		beginRequest(scriptFilename, null, keepAlive);
	}

	public void beginRequest(String scriptFilename, String queryString,
			boolean keepAlive) throws IOException {
		socketChannel.write(new FCGIBeginRequest(FCGIRole.RESPONDER, keepAlive)
				.getByteBuffers());
	...
	public void abortRequest() throws IOException {
		socketChannel
				.write(new FCGIHeader(FCGIHeaderType.FCGI_ABORT_REQUEST, 0)
						.getByteBuffer());
	...
	private void flushParams() throws IOException {
		...
		collectBuffers(FCGIParams.NULL.getByteBuffers(), bufferArrays,
				buffersSize);
	...
	private void readyInternalParams() {
		addParams(PARAM_SCRIPT_FILENAME, getScriptFilename());

		String myQueryString = getQueryString();
		if (myQueryString != null) {
			addParams(PARAM_QUERY_STRING, myQueryString);
		}

		String myRequestMethod = getRequestMethod().toUpperCase();

		addParams(PARAM_REQUEST_METHOD, myRequestMethod);

		if (myRequestMethod.equals("POST")) {
			addParams(PARAM_CONTENT_TYPE, getContentType());
			addParams(PARAM_CONTENT_LENGTH, String.valueOf(getContentLength()));
		}
	}

	private void collectBuffers(ByteBuffer[] buffers,
			ArrayList<ByteBuffer[]> bufferArrays, int[] buffersSize) {
	...
	private void readyWrite() throws IOException {
		...
		if (stdinsFlushed) {
			throw new FCGIException(
					"stdin has flushed and lock, can't be writen");
		}
	...
	public long read(ByteBuffer[] dsts, int offset, int length)
		...
		outer: for (int i = offset; i < maxLength; i++) {
			...
			while (dst.hasRemaining()) {
				...
				if (header.getType() == FCGIHeaderType.FCGI_STDOUT
						&& header.getLength() != 0) {
					int currentRead = readStdoutData(dst, header.getLength(),
							header.getPadding());
		...
		while (dst.hasRemaining()) {
			...
			if (header.getType() == FCGIHeaderType.FCGI_STDOUT
				...
				if (currentRead < header.getLength()) {
					bufferStdoutData(header.getLength() - currentRead,
							header.getPadding());
				}
	...
	private void readyRead() throws IOException {
		if (!readStarted) {
			...
			if (firstHeader.getType() == FCGIHeaderType.FCGI_STDOUT) {
				bufferStdoutData(firstHeader.getLength(),
						firstHeader.getPadding());
				readResponseHeaders();
			}
	...
	private int readStdoutData(ByteBuffer buffer, int available, int padding)
			throws IOException {
